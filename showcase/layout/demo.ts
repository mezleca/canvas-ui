import { CanvasRenderer, UI, FlexLayout, ScrollBehavior, CheckboxWidget, SliderWidget } from "../../index.ts";
...
const main = ui.create_flex({ w: 1200, h: 800, direction: "row", wrap: true, gap: 20 });
...
const create_title = (text: string, parent: FlexLayout) => {
    ui.create_text({ text, font: "Arial", size: 18, color: { r: 255, g: 255, b: 255, a: 255 }, parent });
};

// panel 1: column layout with scroll
const panel1 = ui.create_flex({ w: 360, h: 100, direction: "column", wrap: false, gap: 10 });
...
for (let i = 0; i < 20; i++) {
    const row = ui.create_flex({ w: 330, h: 40, direction: "row", justify: "space-between", align: "center" });
...
const panel2 = ui.create_flex({ w: 360, h: 760, direction: "column", wrap: false, gap: 15 });
...
const wrap_container = ui.create_flex({ w: 330, h: 300, direction: "row", wrap: true, gap: 10 });
...
for (const color of wrap_colors) {
    const box = ui.create_box({ w: 95, h: 80, bg: color, radius: 8 });
...
// buttons section
const buttons_section = ui.create_flex({ w: 330, h: 100, direction: "row", wrap: true, gap: 10, justify: "center" });
...
const panel3 = ui.create_flex({ w: 360, h: 760, direction: "column", wrap: false, gap: 15 });
...
for (const mode of justify_modes) {
    ui.create_text({ text: mode, size: 12, color: { r: 140, g: 140, b: 140, a: 255 }, parent: panel3 });

    const row = ui.create_flex({ w: 330, h: 50, direction: "row", justify: mode, align: "center" });
    row.style.background_color({ r: 50, g: 50, b: 55, a: 255 });
    row.style.border_radius(6);
    row.style.padding(8);

    for (let i = 0; i < 3; i++) {
        ui.create_box({ w: 60, h: 34, bg: { r: 70, g: 130, b: 180, a: 255 }, radius: 4, parent: row });
    }
...
const panel4 = ui.create_flex({ w: 300, h: 760, direction: "column", gap: 10, wrap: false });
...
const dynamic_container = ui.create_flex({ w: 270, h: 400, direction: "row", wrap: true, gap: 10 });
...
// controls
const controls = ui.create_flex({ w: 270, h: 100, direction: "row", wrap: true, gap: 10 });
...
const btn_add = ui.create_button({ text: "add item" });
const btn_remove = ui.create_button({ text: "remove last" });

btn_add.on_click(() => {
    const item = ui.create_button({ text: "item " + (dynamic_container.children.length + 1) });
...
const btn_cat = ui.create_button({ text: "spawn cat" });
btn_cat.on_click(() => {
    const cat = ui.create_image({ src: cat_img, w: 60, h: 60 });
...
const anim_box = ui.create_box({ w: 100, h: 60, radius: 8, border: 2, border_color: { r: 255, g: 255, b: 255, a: 100 } });
...
const panel5 = ui.create_flex({ w: 300, h: 760, direction: "column", wrap: false, gap: 12 });
...
const helper_row = ui.create_flex({ w: 260, h: 40, direction: "row", gap: 10, align: "center" });
helper_row.style.background_color({ r: 50, g: 50, b: 55, a: 255 });
helper_row.style.border_radius(6);
helper_row.style.padding(8);

ui.create_text({ text: "factory text", size: 12, color: { r: 180, g: 180, b: 180, a: 255 }, parent: helper_row });
ui.create_box({ w: 16, h: 16, bg: { r: 90, g: 140, b: 200, a: 255 }, radius: 3, parent: helper_row });

panel5.add_children(helper_row);

const control_row = ui.create_flex({ w: 260, h: 80, direction: "row", gap: 12, align: "center", wrap: true });
...
const radio_row = ui.create_flex({ w: 260, h: 40, direction: "row", gap: 10, align: "center" });
